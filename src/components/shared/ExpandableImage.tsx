...
const ExpandableImage = ({
  ...
  return <>
      ...
      {isExpanded && <div className="fixed inset-0 bg-black/50 z-[999] sm:hidden flex items-center justify-center" onClick={handleToggle}>
          <div className="relative w-[90%] max-h-[90%] bg-white rounded-xl p-4 shadow-lg" onClick={e => e.stopPropagation()}>
      ...
      {/* Normal Image View */}
      <div className="w-full cursor-pointer transition-transform duration-200 active:scale-95 sm:active:scale-100" onClick={() => {
      if (window.innerWidth < 640) {
        // Only trigger on mobile
        handleToggle();
      }
    }}>
        <img src={src} alt={alt} className="w-full h-auto rounded-lg shadow-md object-fill" />
      </div>
    </>;
};
