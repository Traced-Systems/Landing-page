...
const TabsSection = () => {
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden h-[200px] md:h-[200px]">
        ...
        <TabsList className="flex md:flex-col md:h-full md:w-[40%] space-y-0 bg-[#F5F5F5] rounded-none p-0 flex-row w-full">
          {["mission", "focus", "solution"].map(tab => (
            <TabsTrigger 
              key={tab} 
              value={tab} 
              className="w-full flex-1 flex items-center px-6 py-2 md:py-4 text-left justify-start rounded-none border-l-4 border-transparent data-[state=active]:bg-[#204850] data-[state=active]:text-white data-[state=active]:border-[#204850] transition-all"
            >
            ...
            >
              <div className="relative h-full">
                <img 
                  ...
                  alt={tab} 
                  className="w-full h-full object-cover brightness-[0.3] md:brightness-100" 
                />
                <div className="absolute inset-0 bg-[#204850]/90 md:bg-black/60 flex items-start md:items-center justify-center p-4 md:p-8 overflow-y-auto">
                  <p className="text-white text-sm md:text-base font-normal md:text-center text-left leading-relaxed">
                    {tab === "mission" ? (
                      <>
                        <strong className="block mb-2 text-base md:text-base text-[#9FE8FA]">Our mission is to transform supply chain efficiency and engagement by leveraging blockchain technology to enhance transparency, collaboration, and customer loyalty.</strong> From raw materials to recycling, we empower businesses to optimize workflows, build stronger brand connections, and deliver sustainable value to every stakeholder in the chain.
                      </>
                    ) : tab === "focus" ? (
                      <>
                        <strong className="block mb-2 text-base md:text-base text-[#9FE8FA]">Our focus is on helping companies stay ahead of evolving regulations while ensuring a tangible return on investment.</strong> We guide organizations in targeting and engaging the right audiences (from legislators to end consumers) across both B2B and B2C environments. By employing a progressive, future-proof approach, we enable companies to meet (and exceed) their sustainability goals without compromising budgets or growth.
                      </>
                    ) : (
                      <>
                        <strong className="block mb-2 text-base md:text-base text-[#9FE8FA]">Our platform goes beyond mere compliance.</strong> By identifying and collaborating with key partners in the value chain, we help businesses integrate a truly circular approach that aligns with European values and environmental objectives. Whether you're seeking to reduce waste, boost resource efficiency, or enhance brand loyalty, our solution puts you at the forefront of sustainable innovation today and into the future.
                      </>
                    )}
                  </p>
                </div>
